...
\begin{figure}[htpb]
  ...
  \end{tikzpicture}
  \caption{
Generative ML Model as a channel $\mathcal{C} = Next_{\mathcal{GPT}}$ used in the Meteor stegosystem. $Next_{\mathcal{GPT}}$ takes a history $h \in \mathcal{H}$ as input and outputs a tuple $(\mathcal{T}, \mathcal{P})$, where $\mathcal{P} = \{p_1, p_2, \dots, p_{|\mathcal{T}|}\}$ is a probability distribution over tokens $\mathcal{T} = \{ t_1, t_2, \dots, t_{|\mathcal{T}|}\}$. 
...
In \autoref{chap:reliability}, we will argue that the Meteor stegosystem is in fact not reliable using GPT-2 as a channel and propose a modification to Meteor that improves reliability while -- in the worst case -- introducing computational overhead exponential in stegotext length.
...
In the paper, it is argued that a steganographic protocol is secure if no ppt. adversary $\mathcal{A}$ is able to distinguish the output of the stegosystem from the output of a random oracle $\mathcal{O}$ that samples from the distribution.
