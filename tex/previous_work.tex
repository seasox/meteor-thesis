...
\begin{definition}[Ranged Randomness Recoverable Sampling Scheme, \cite{Meteor2021}]
  ...
  \begin{enumerate}
    \item $Sample_{\mathcal{M}}^\beta(h, r)$ on history $h \in \mathcal{H}$ and randomness $r \in \{0,1\}^\beta$, sample an output $s$ from the underlying distribution $\mathcal{M}$
    \item $Recover_{\mathcal{M}}^\beta(h, s)$ on history $h \in \mathcal{H}$ and sample $s \in \Delta$ returned from $Sample_{\mathcal{M}}^\beta$, output the set $\mathcal{R} = \{ r \in \{0,1\}^\beta | Sample_{\mathcal{M}}^\beta(\mathcal{H}, r) = s \}$ of possible values for $r$
  \end{enumerate}
\end{definition}
