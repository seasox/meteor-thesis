...
If the warden can distinguish the stegotext from a covertext, they have an advantage against the stegosystem in the distinguishing game.
...
where $\mathcal{W}(t, q, l)$ denotes the set of all adversaries which make at most $q(k)$ queries totalling at most $l(k)$ bits (of hiddentext) and running in time at most $t(k)$.
\end{definition}
...
The model's output is a logits vector $z$.
The logits vector is a vector of unweighted activations of the model's underlying layers, where a higher value represents a stronger activation.
Ultimately, we want to have a probability distribution to sample from.
To transform the logits vector to a probability distribution $p = (p_i)_{1 \leq i \leq n}$ with $\sum_{i=1}^n p_i = 1$, we can use the softmax function $\sigma$:
...
RRRSS Sample Algorithm for GPT.
...
\begin{Pseudocode}[caption={
RRRSS Recover Algorithm for GPT
