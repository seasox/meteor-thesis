...
\begin{tikzpicture}
	...
	\node[below=of alice] (h) {$\mathcal{H}$};
	\node[right=of bob] (mb) {m};
	\node[above=of bob] (kb) {k};
	\node[below=of bob] (hb) {$\mathcal{H}$};
	\node[block, below=of warden] (steganalysis) {Steganalysis: is $d$ stegotext?};
	...
	\draw[->] (steganalysis) -- node[right] {Yes} (isstego);
	\draw[->] (steganalysis) -- node[left] {No} (notstego);
\end{tikzpicture}
\caption{
...
\lstinputlisting[float, language=Python, caption={Python program which generates text using the Huggingface Transformers Pipeline API}, label={code:gpt2-demo}]{code/pipeline_gpt2.py}

\lstinputlisting[float, language=Python, caption={Python program which generates text using the Huggingface Transformers}, label={code:gpt2-logits-demo}]{code/logits_demo_gpt2.py}
