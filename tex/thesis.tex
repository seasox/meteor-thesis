...
\begin{definition}
(Steganographic Secrecy, Hopper) A Stegosystem $\mathcal{S}_k$ is ca	lled $(t,q,l,\epsilon)$ steganographically secret against chosen hiddentext attack for the channel $\mathcal{C}$ ($(t,q,l,\epsilon)$-SS-CHA-$\mathcal{C}$) if $InSec_{\mathcal{S},\mathcal{C}}^{ss}(t,q,l,k) \leq \epsilon$
\end{definition}
...
\begin{definition}
(Universal Steganographic Secrecy, Hopper) A Stegosystem $\mathcal{S}$ is called $(t, q, l, \epsilon)$-universally steganographic secret agains chosen hiddentext attack ($(t,q,l,k,\epsilon)$-SS-CHA if it is $(t,q,l,\epsilon)$-SS-CHA-$\mathcal{C}$ for every always-informative channel $\mathcal{C}$
\end{definition}
...
In their paper \cite{Meteor2020} Kaptchuk et al. present the Meteor stegosystem, a provably secure steganographic system for realistic distributions as approximated by ML models.
...
\begin{definition}
	\label{def:rrrss}
	(Ranged Randomness Recoverable Sampling Scheme, Kaptchuk et al.) 
...
They introduce another formal definition:

\begin{definition}
(Correctness, Kaptchuk et al.) A steganographic protocol must be correct, i.e. except with negligible probability an encoded message can be recovered using the decode algorithm. Formally, for any $k \leftarrow KeyGen(1^\lambda)$,

$$Pr[Decode_{\mathcal{D}}(k, Encode_{\mathcal{D}}(k, m, \mathcal{H}), \mathcal{H}) = m] \geq 1 - negl(\lambda)$$
\end{definition}
...
They argue that a steganographic protocol is secure if no ppt. adversary $\mathcal{A}$ is able to distinguish the output of the stegosystem from the output of a covertext distribution, i.e. an oracle $\mathcal{O}$ sampling from the distribution. More formally, they define security as follow:

\begin{definition}
	\label{def:sec-kaptchuk}
	(Security, Kaptchuk et al.) We say that a steganographic scheme $\Sigma_{\mathcal{D}}$ is secure against chosen hiddentext attacks if for all ppt. adversaries $\mathcal{A}_{\mathcal{D}}$,
	
	$$k \leftarrow KeyGen_{\mathcal{D}}(1^\lambda)$$
	$$\left| Pr\left[ \mathcal{A}_{\mathcal{D}}^{Encode_{\mathcal{D}}(k, \cdot, \cdot)}=1 \right] - \left[ \mathcal{A}_{\mathcal{D}}^{\mathcal{O}_{\mathcal{D}}(\cdot, \cdot)}=1 \right] \right| < negl(\lambda)$$
\end{definition}

Furthermore, they argue that the Meteor stegosystem is secure by reduction to the PRG real-or-random game. 
In \autoref{chap:security}, we will discuss how \autoref{def:sec-kaptchuk} is weaker than Hopper's definition of universaly steganographic secrecy introduced in \autoref{sec:prov-sec-steg}.
We will also discuss how an attacker can distinguish Meteor's output from random oracle using polynomial queries, time and message length.
