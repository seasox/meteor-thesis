...
First, we will discuss steganography and provably secure steganography, introduced by Hopper et. al. \todo{add bibtex}
...
There, we embed a fixed amount of bits in each message, using the DialoGPT model developed by Microsoft \todo{bibtex} to generate dialog-like messages.
...
In his doctoral thesis ``Toward a theory of Steganography'', Nicholas J. Hopper \todo{bibtex} establishes a cryptographic point of view on steganography. 
...
$$\mathop{Adv}_{\mathcal{S}, \mathcal{C}, W}^{ss}(k) = |Pr[W^{ST}(1^k)=1] - Pr[W^{CT}(1^k)=1]|$$ \todo{why above/below?}
